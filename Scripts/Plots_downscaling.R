...
g = ggplot(com_poly_t) + geom_sf(aes(fill = Moran), color = NA)  + scale_fill_manual(name = "",values = colors,breaks = c("high-high")) +   geom_sf(data = prov_poly, fill = NA) + theme_pubr(base_size = 20)+ theme(legend.key = element_rect(color="black"))+ theme(legend.key = element_rect(color="black"),legend.key.size = unit(2.5,"line"))  +
