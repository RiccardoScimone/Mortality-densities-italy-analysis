...
for ( class in classe){
  ...
  plot_prov = ggplot(plot_provs, mapping = aes(x = date, y = deaths, color = name) )+ colScale +   geom_line(aes(alpha  = ifelse(name %in% c("Roma", "Milano", "Bergamo","Napoli"), 1, 0.5))) + geom_line(aes(y = md, x = date, color = year), data = means_provs, col = "black", lwd = 1.4) +
    facet_grid(~Year) +scale_x_date(date_labels = "%b") + theme_pubr(base_size = 50)  + rremove("legend")+ rremove("xlab") + rremove("ylab")  + labs_pubr(base_size = 40) + font("xy.text",size = 25)
#  x11()
#  plot(plot_prov)
  ...
  means_provs = plot_provs %>% left_join(provinces_data %>% filter(CL_ETA == class) %>% dplyr::select(provincia,num_residenti), by = c("name" = "provincia")) %>% ungroup() %>% group_by(date, Year) %>% summarise(md = sum(deaths*num_residenti)/tot_res)
  plot_prov = ggplot(plot_provs, mapping = aes(x = date, y = deaths, color = name) )+ colScale +   geom_line(aes(alpha  = ifelse(name %in% c("Roma", "Milano", "Bergamo","Napoli"), 1, 0.5))) + geom_line(aes(y = md, x = date, color = year), data = means_provs, col = "black", lwd = 1.4) +
    facet_wrap(~Year, nrow = 1) +scale_x_date(date_labels = "%b") + theme_pubr(base_size = 40)  + rremove("legend")+ rremove("xlab") + rremove("ylab")  + labs_pubr(base_size = 50) + font("xy.text",size = 25)
  #  x11()
  #  plot(plot_prov)
  ...
  ### Normalize by residents
  plot_provs = provinces_aggregated %>% filter(CL_ETA == class) %>% ungroup() %>% dplyr::select(NOME_PROVINCIA,partial_date_death,T_17,T_18, T_19, T_20)
  names(plot_provs) = c("name","date", "2017","2018", "2019", "2020")
  plot_provs = plot_provs %>% pivot_longer(cols = c("2017","2018", "2019","2020"), names_to = "Year", values_to = "deaths") %>% filter(!(date == "2020-02-29" & Year != "2020"))
  colors = rep("gray",107)
  plot_provs = plot_provs %>% left_join(provinces_data %>% filter(CL_ETA == class) %>% dplyr::select(provincia, num_residenti), by = c("name" = "provincia"))
  plot_provs$name = factor(plot_provs$name, levels = c(setdiff(plot_provs$name, c("Roma", "Milano", "Bergamo","Napoli") ), c("Roma", "Milano", "Bergamo","Napoli")))
  names(colors) = levels(factor(plot_provs$name))
  colors[["Roma"]] = "purple"
  colors[["Milano"]] = "green"
  ...
  colors[["Napoli"]] = "blue"
  colScale = scale_colour_manual(name = "name", values = colors)
  means_provs = plot_provs  %>% ungroup() %>% group_by(date, Year) %>% summarise(md = sum(deaths)/tot_res)
  plot_prov = ggplot(plot_provs, mapping = aes(x = date, y = deaths/num_residenti, color = name) )+ colScale +   geom_line(aes(alpha  = ifelse(name %in% c("Roma", "Milano", "Bergamo","Napoli"), 1, 0.5))) + geom_line(aes(y = md, x = date, color = year), data = means_provs, col = "black", lwd = 1.4) +
    facet_wrap(~Year, nrow = 1) +scale_x_date(date_labels = "%b") + theme_pubr(base_size = 40)  + rremove("legend")+ rremove("xlab") + rremove("ylab")  + labs_pubr(base_size = 50) + font("xy.text",size = 25)
  #  x11()
  #  plot(plot_prov)
  pdf(paste0("Output/Plot/",folder_names[[class]],"/Paper_data_presentation3.pdf"), width = 40, height = 20)
  plot(plot_prov)
  dev.off()
...
load("Output/Data/Exploratory_plots_provinces.Rdata")
...
g = ggarrange(  
            annotate_figure(ggarrange(plotlist = list(plot_list_provinces_heatmaps[[class]]$T_17,plot_list_provinces_heatmaps[[class]]$T_18,plot_list_provinces_heatmaps[[class]]$T_19,plot_list_provinces_heatmaps[[class]]$T_20), common.legend = T, nrow = 1, labels = NULL,  legend = "bottom"),top = text_grob("Incidence of mortality, 70+ years",face = "bold", size = 30)),
            annotate_figure(ggarrange(plotlist = list(plot_list_provinces_moran[[class]]$T_17,plot_list_provinces_moran[[class]]$T_18,plot_list_provinces_moran[[class]]$T_19,plot_list_provinces_moran[[class]]$T_20), nrow = 1, labels = NULL,common.legend = T, legend = "none"),top = text_grob("\n Spatial clusters of incidence of mortality, 70+ years, Provinces",face = "bold", size = 30)),
            annotate_figure(ggarrange(plotlist = list(plot_list_municipalities_moran[[class]]$T_17,plot_list_municipalities_moran[[class]]$T_18,plot_list_municipalities_moran[[class]]$T_19,plot_list_municipalities_moran[[class]]$T_20), nrow = 1, labels = NULL, common.legend = T, legend = "bottom"),top = text_grob("Spatial clusters of incidence of mortality, 70+ years, Municipalities",face = "bold", size = 30) ),
...
for ( class in classe)  {
  ...
  g = prov_poly_t %>% ggplot() + geom_sf(aes(fill = log(distance))) + scale_fill_viridis(option = "inferno") + 
    ...
    geom_text(data = dum_corr, aes(x = x, y = y, label = paste0("R = ", round(R,4))), size = 4) + 
    facet_wrap(.~ window,nrow = 2) + clean_theme()+theme_pubclean(base_size = 20) + labs_pubr(base_size = 20) + theme(legend.position="bottom") + rremove("xlab") + rremove("ylab") + rremove("xy.text") + rremove("ticks") 
...
for( class in classe){
  ...
  for ( year in years_to_plot){
    ...
    plotlist[[year]] = plotlist[[year]] + ylim(4,16) + theme_pubr(base_size = 20)   + rremove("legend")+ rremove("xlab") + rremove("ylab")  + labs_pubr(base_size = 20) + font("xy.text",size = 17) + theme(aspect.ratio=1)
  }
  plot(ggarrange(plotlist = plotlist,nrow = 1,ncol = 4))
  dev.off()
}
