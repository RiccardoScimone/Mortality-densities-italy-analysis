...
for(class in classe){
  ...
  for(year in years){
    ...
    g = ggplot(com_poly_t) + geom_sf(aes(fill = Moran), color = NA) + 
      ...
      scale_fill_manual(name = "",values = colors) + theme_pubr() +theme(text = element_text(size=30),legend.key.size = unit(3,"line") ) + geom_sf(data = it_poly, fill = NA) + rremove("axis") + rremove("axis") + rremove("xy.title") + rremove("axis.text") + rremove("ticks") 
